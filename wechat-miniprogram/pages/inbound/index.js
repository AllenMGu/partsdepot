...
Page({
  ...
  onSupplierInput(e) {
    this.setData({ "createForm.supplier": e.detail.value });
  },
  ...
  async loadOrders() {
    ...
    try {
      const res = await request({ url: "/inbound-orders/" });
  ...
  async createOrder() {
    ...
    try {
      const body = {
        supplier: this.data.createForm.supplier,
        remark: this.data.createForm.remark
      };
      await request({ url: "/inbound-orders/", method: "POST", data: body });
      wx.showToast({ title: "创建成功", icon: "success" });
      this.setData({ createForm: { supplier: "", remark: "" } });
      await this.loadOrders();
    } catch (err) {
  ...
  async openDetail(e) {
    ...
    try {
      const detail = await request({ url: `/inbound-orders/${id}` });
  ...
  async addItem() {
    ...
    try {
      await request({
        url: `/inbound-orders/${orderId}/items`,
        method: "POST",
        data: {
  ...
  async deleteItem(e) {
    ...
    this.setData({ loadingAddItem: true });
    try {
      await request({ url: `/inbound-orders/${orderId}/items/${itemId}`, method: "DELETE" });
  ...
  async submitOrder() {
    ...
    try {
      await request({ url: `/inbound-orders/${orderId}/submit`, method: "POST" });
  ...
  async deleteOrder() {
    ...
    try {
      await request({ url: `/inbound-orders/${orderId}`, method: "DELETE" });
      wx.showToast({ title: "已删除", icon: "success" });
      this.setData({ selected: {} });
      await this.loadOrders();
    } catch (err) {
  ...
  async returnOrder() {
    const orderId = this.data.selected.id;
    if (!orderId) return;
    this.setData({ loadingReturn: true });
    try {
      const res = await request({ url: `/inbound-orders/${orderId}/return`, method: "POST" });
      wx.showToast({ title: `已生成${res.order_no}`, icon: "none" });
      await this.loadOrders();
    } catch (err) {
      wx.showToast({ title: err.message || "退库失败", icon: "none" });
    } finally {
      this.setData({ loadingReturn: false });
    }
  }
});
